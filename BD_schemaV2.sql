...
--
-- Client :  127.0.0.1
-- Généré le :  Jeu 12 Février 2015 à 18:42
...
('001', '002', 'Où est morte la victime?', 'SELECT ?loc  WHERE { 	?death gs:eventPlace ?place. 	?place v:location-name ?loc. }'),
('001', '003', 'Quelle identité de la victime?', 'Select ?familyName ?givenName ?name  WHERE { 	?death gs:deceaded ?p. 	\r\n?p foaf:family-name ?familyName. 	\r\n?p foaf:given-name ?givenName. }'),
('001', '004', 'Quel est l''âge de la victime?', 'SELECT ?age  WHERE { 	?death gs:deceaded ?p. 	?p foaf:age ?age. }'),
('001', '005', 'Avec quel instrument été tué la victime?', 'SELECT ?instrument  WHERE { 	?v rdf:type gs:ViolentAct. 	?v gs:instrument ?instrument. }'),
('001', '006', 'Qui a déclaré la mort de l''homme?', 'SELECT ?name ?person WHERE { 	?declaration gs:content ?death. 	?death rdf:type gs:Death. 	?declaration gs:hasAgent ?person. 	?person foaf:name ?name. }'),
('001', '007', 'Comment est morte la victime?', 'Select ?cause  WHERE { 	?death gs:deceaded ?person. 	?death gs:event-cause ?v. 	?v gs:violentAct-type ?cause. }'),
...
('002', '003', 'Que ont fait les Français? ', 'Select ?action  WHERE { 	?a gs:isAgent ?agent. 	?agent gs:nat "french". 	?a rdf:type ?action }'),
